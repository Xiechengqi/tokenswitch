...
export function MapSection({ locale }: { locale: Locale }) {
  ...
  return (
    <section className="bg-terminal py-16 text-white">
      <div className="mx-auto max-w-[var(--container)] px-4 sm:px-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="font-heading text-3xl font-bold">{t.map.title}</h2>
            <p className="mt-2 text-white/70">{t.map.subtitle}</p>
          </div>
          <Button href={localePath(locale, "network")} variant="secondary">
            {t.nav.network} →
          </Button>
        </div>
        <div className="mt-8 overflow-hidden rounded-2xl border-2 border-white/15">
